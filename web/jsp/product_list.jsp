<%@ page import="com.like.domain.PageBean" %>
...
<div style="width:380px;margin:0 auto;margin-top:50px;">
    <ul class="pagination" style="text-align:center; margin-top:10px;">
        <c:if test="${list.currentPage != 1}">
            <li>
                <a
                        href="${pageContext.request.contextPath}/product?method=findByCategory&page=${list.currentPage - 1}&id=${id}"
                        aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
        </c:if>
        <c:forEach begin="1" end="${list.totalPage}" var="p">
            <c:if test="${list.currentPage == p}">
                <li class="active">

                    <a
                            href="${pageContext.request.contextPath}/product?method=findByCategory&page=${p}&id=${id}">${p}
                    </a>
                </li>
            </c:if>
            <c:if test="${list.currentPage != p}">
        ...
        <li>
            <c:if test="${list.currentPage != list.totalPage}">
                <a href="${pageContext.request.contextPath}/product?method=findByCategory&page=${list.currentPage + 1}&id=${id}"
                   aria-label="Next">
